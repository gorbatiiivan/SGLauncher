...
uses
  ...
  Vcl.Imaging.GIFImg, Xml.XMLIntf, Xml.XMLDoc, Math, ActiveX, ShellAPI, IniFiles,
  Vcl.Menus, ShlObj, Vcl.ImgList, StrUtils, System.Generics.Collections;
...
var
  AppPath, BasePath, ExtrasPath, FullExtrasPath: string;
  MenuItem: TMenuItem;
  ...
  LargeIcon: HICON;
begin
  // Очищаем предыдущие пункты меню
  while PopupMenu1.Items.Count > 6 do
  ...
  Files := TDirectory.GetFiles(FullExtrasPath, '*.*',
                                TSearchOption.soTopDirectoryOnly);

  // Создаем и настраиваем ImageList для 32-битных иконок
  if not Assigned(PopupMenu1.Images) then
  begin
    PopupMenu1.Images := TImageList.Create(PopupMenu1);
    // Включаем поддержку 32-битных цветов
    PopupMenu1.Images.ColorDepth := cd32Bit;
    // Устанавливаем размер иконок (например, 16x16 или 32x32)
  ...
  begin
    ...
    MenuItem.Hint := FileName; // Полный путь к файлу
    MenuItem.OnClick := OnExtrasMenuItemClick;

    // Получаем 32-битную иконку из системы (большую иконку)
    // Используем SHGFI_ICON для получения дефолтной иконки или
    // SHGFI_LARGEICON для большой иконки, которая обычно содержит больше деталей
    ...
    begin
      try
        ...
        try
          ...
          // Альтернативный способ - использовать ExtractIconEx для получения 32-битной иконки
          // ExtractIconEx(PChar(FileName), 0, nil, @LargeIcon, 1);
          // if LargeIcon <> 0 then
          // begin
          ...
          // Важно: уничтожаем иконку, полученную из SHGetFileInfo
          DestroyIcon(FileInfo.hIcon);
          // Если используете ExtractIconEx, уничтожьте и его:
        ...
        begin
          ...
          // ShowMessage('Error loading icon: ' + E.Message);
        end;
      end;
    end
    else
    begin
      // Не удалось получить иконку, оставляем без иконки
...
begin
  ...
  ResizeLabelToText(Label1);
  NextImgBtn.Top := Panel2.Height div 2 - 12;
  NextImgBtn.Left:= Panel2.Width - NextImgBtn.Width - 6;
  PrevImgBtn.Top := Panel2.Height div 2 - 12;
end;
...
begin
 if ListView1.ItemIndex <> -1 then
  begin
   PathGame := LaunchBoxDir +'\'+ListView1.Selected.SubItems[0];
